...
```markdown
# Hosting a Static Website on Google Cloud Storage with Cloudflare SSL Proxy

This guide provides step-by-step instructions for hosting a static website using Google Cloud Storage (GCS) and proxying it through Cloudflare for free automatic SSL termination.
...
When using Cloudflare CNAME proxying directly to Cloudflare, **the GCS bucket name must match your exact domain/subdomain name** (e.g., `www.example.com`).
...
# 3. Configure bucket as a static website
gcloud storage buckets update gs://$DOMAIN --web-main-page-suffix=index.html
...
Once the DNS changes propagate, test your deployment:
